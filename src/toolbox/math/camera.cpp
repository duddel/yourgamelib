...
namespace yourgame
{
    namespace math
    {
        ...
        void Camera::setProjection(math::Projection proj)
        {
            if (proj != m_proj)
            {
                m_proj = proj;
        ...
        void Camera::setPerspective(float fovy, float aspect, float zNear, float zFar)
        {
            ...
            m_zNear = zNear;
            m_zFar = zFar;
            m_proj = math::Projection::PERSPECTIVE;
            m_pMatInvalidated = true;
        }
        ...
        void Camera::setOrthographic(float height, float aspect, float zNear, float zFar)
        {
            ...
            m_zNear = zNear;
            m_zFar = zFar;
            m_proj = math::Projection::ORTHOGRAPHIC;
            m_pMatInvalidated = true;
        }
        ...
        {
            m_fovy = fovy;
            if (m_proj == math::Projection::PERSPECTIVE)
            {
                m_pMatInvalidated = true;
            }
        }
        ...
        {
            m_height = height;
            if (m_proj == math::Projection::ORTHOGRAPHIC)
            {
                m_pMatInvalidated = true;
            }
        }
        ...
        void Camera::castRay(float viewportX, float viewportY, glm::vec3 &dstOrg, glm::vec3 &dstDir)
        {
            ...
            else
            {
                ...
                dstDir = -m_trafo.getAxisLocal(yourgame::math::Axis::Z);
            }
        }
